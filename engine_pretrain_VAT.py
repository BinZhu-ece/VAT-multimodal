...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if not args.use_amp:# not args.use_amp
            ...
            # assert torch.isnan(model.mu).sum() == 0, 'model.parameters is nan'
            # for name,value in model.named_parameters():
            #     try:
            #         print('111,name:{},value:max_{}_min_{},grad_value:max_{}_min_{}'.format(name,torch.max(value),torch.min(value),torch.max(value.grad),torch.min(value.grad)))
            #     except:
            #         pass
            if update_grad:
            ...
            # for name,value in model.named_parameters():
            #     try:
            #         print('222,name:{},value:max_{}_min_{},grad_value:max_{}_min_{}'.format(name,torch.max(value),torch.min(value),torch.max(value.grad),torch.min(value.grad)))
            #     except:
            #         pass
