...
def get_args_parser():
    ...
    parser.add_argument('--lmdb_vat_folder', default='lmdb_you_0_5',type=str,
                        help='')
    parser.add_argument('--part', default='0',type=str,
                        help='')
    #------------------------ lmdb over! ----------------------------------------------------
...
def main(args):
    ...
    def tokenize_function2(data_args, rela_path):
        text = open(opj(data_args.text_root, rela_path), 'r').read() 
        first_line = text.split('\n')[0]

        def dur_search(video_url):
            return float(video_url.split('dur=')[1].split('&lmt=')[0])
        def cls_search(first_line):
            title = text.split('#')[0]

        title = text.split('#')[0]
        duration = dur_search(text.split('\n')[1])
        ...
        return 'text@{}'.format(id), pixel_values['input_ids'], duration, first_line, id 
    ...
    per_rank_read(args.rank, per_rank_num, pros=40)
